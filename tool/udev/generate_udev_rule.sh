...
for bdf in $(iter_all_pci_class_code 01 08 02); do
		...
		if ! $mount; then
			echo "ACTION==\"bind\", KERNELS==\"$bdf\" , SUBSYSTEMS==\"pci\", DRIVERS==\"nvme\", ATTRS{class}==\"0x010802\" , \\
...
" >> $rootdir/tool/udev/99-custom-nvme.rules
			echo "$bdf : Added to Udev Rule File"
		else
			for name in ${blknames[@]}; do
				echo "$bdf : Active mountpoints on /dev/$name, so not adding to Udev Rule"
			done
		fi
done