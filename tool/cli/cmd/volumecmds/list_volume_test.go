...
func TestListVolumeCommandReq(t *testing.T) {
	...
	testmgr.ExecuteCommand(rootCmd, "volume", "list", "--array-name", "Array0", "--json-req")

	output := buff.String()
	output = output[:len(output)-1] // Remove the last n from output string
	expected := `{"command":"LISTVOLUME","rid":"fromfakeclient","param":{"array":"Array0"}}`
