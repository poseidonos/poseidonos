...
func TestUnmountVolumeCommandReq(t *testing.T) {
	...
	testmgr.ExecuteCommand(rootCmd, "volume", "unmount", "--volume-name", "vol01", "--array-name", "Array0", "--json-req")

	output := buff.String()
	output = output[:len(output)-1] // Remove the last n from output string
	expected := `{"command":"UNMOUNTVOLUME","rid":"fromfakeclient","param":{"name":"vol01","array":"Array0"}}`
