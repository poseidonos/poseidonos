...
func TestRemoveSpareCommandReq(t *testing.T) {
	...
	testmgr.ExecuteCommand(rootCmd, "array", "rmspare", "--spare", "device0", "--array-name", "Array0", "--json-req")

	output := buff.String()
	output = output[:len(output)-1] // Remove the last n from output string
	expected := `{"command":"REMOVEDEVICE","rid":"fromCLI","param":{"array":"Array0","spare":[{"deviceName":"device0"}]}}`
