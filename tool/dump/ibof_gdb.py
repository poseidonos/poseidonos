...
class IbofosGdbCmd(gdb.Command):  
    ...
    def show_dev_context(self, ublock_addr):
        ...
        output = gdb.execute("p ((pos::UBlockDevice *) "+ ublock_addr +")->property", to_string=True) 
        print (output)

        output = gdb.execute("p ((pos::UBlockDevice *) "+ ublock_addr +")->contextArray", to_string=True) 
        ...
        for mem in memaddr:
            ...
            output = gdb.parse_and_eval('((pos::UnvmeDeviceContext *)'+ memhex +')->pendingIoCount')
                    
            print ('\t=== Thread %d\'s Pending Io ===' % (index))
            print ('================================================')
            print ("pending IO count : ", output)
            
            output = gdb.parse_and_eval('((pos::UnvmeDeviceContext *)'+ memhex +')->pendingErrorList')
    ...
    def show_pending_io(self):
        ...
        output = gdb.execute('p pos::debugInfo->deviceManager->devices', to_string=True)             
    ...
    def show_pending_ubio(self):
        ...
        for line in lines:
            if ("buffer" in line):
                ...
                output_ubio = gdb.execute ("p *((Ubio *)%s)" % (ubio_memaddr), to_string = True).split('\n')
                for line in output_ubio:
                    if ("_vptr.Ubio" in line):
                        if ("pos::VolumeIo" in line):
                            output_rba = gdb.parse_and_eval ("((VolumeIo *) %s)->sectorRba " % (ubio_memaddr))
                        else:
                            output_rba = "Not VolumeIo"
                        break
                output_dir = gdb.parse_and_eval("((Ubio *) %s)->dir " % (ubio_memaddr))
                output_lba = gdb.parse_and_eval ("((Ubio *) %s)->lba " % (ubio_memaddr))
                ...
                print ("ubio address : " + ubio_memaddr + " dir : %s rba : %s deviceLba : %s sync : %s callback pointer : %s" \
    ...
    def check_log_level_debug(self):
            output = str(gdb.parse_and_eval ("pos::debugInfo->logger->preferences->logLevel"))
            print (output)
    ...
    def get_in_memory_log(self):
        ...
        for logger_module in logger_list:
            output = gdb.execute ("p (pos::debugInfo->logger->dumpModule[%d]->dumpQueue)\n" % (index), to_string=True)
            output_list = output.split('\n')
