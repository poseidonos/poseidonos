...
TEST(Allocator, Init_TestInitializeOrNot)
{
    ...
    NiceMock<MockStripeManager>* stripeManager = new NiceMock<MockStripeManager>();
    Allocator alloc(nullptr, addrInfo, ctxManager, blkManager, wbManager, stripeManager, iArrayInfo, iState);
...
TEST(Allocator, Dispose_TestDisposeAfterInitOrNot)
{
    ...
    EXPECT_CALL(*ctxManager, FlushContexts(_, _));
    ...
    EXPECT_CALL(*ctxManager, FlushContexts(_, _)).Times(0);
    // when 2.
    alloc.Dispose();
}
...
TEST(Allocator, GetMeta_TestWBTFunctionsWithType)
{
    ...
    NiceMock<MockContextManager>* ctxManager = new NiceMock<MockContextManager>(nullptr, allocCtx, segCtx,
    ...
    ON_CALL(*ctxManager, GetSegmentCtx).WillByDefault(Return(segCtx));
    ...
    // then 4.
    EXPECT_EQ((int)ERRID(ALLOCATOR_WBT_WRONG_PARAMETER), ret);
