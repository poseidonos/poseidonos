...
bool
IsExisitStripeInList(std::vector<StripeInfo>& targetList, PendingStripe* targetStripe)
{
...
TEST(ActiveWBStripeReplayer, Replay_SingleVolumeMultipleActiveStripe)
{
    ...
    {
        EXPECT_TRUE(IsExisitStripeInList(orphanStripes, stripe));
    }
}
...
TEST(ActiveWBStripeReplayer, Replay_MultiVolumeMultipleActiveStripe)
{
    ...
    for (auto stripe : pendingStripes)
    {
        EXPECT_TRUE(IsExisitStripeInList(activeWbStripes, stripe));
    }
}
