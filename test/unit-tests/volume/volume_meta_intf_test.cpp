...
public:
    ...
    {
        file = new NiceMock<MockMetaFsFileIntf>(FILE_NAME, ARRAY_ID, FILE_TYPE);
        list.Clear();
    }
...
protected:
    ...
    NiceMock<MockMetaFsFileIntf>* file;
...
TEST_F(VolumeMetaIntfFixture, LoadVolumes_testIfAnErrorCodeWillBeReturnedWhenTheFileIsNotExisted)
{
    ...
    EXPECT_EQ(VolumeMetaIntf::LoadVolumes(list, ARRAY_NAME, ARRAY_ID, file), expectValue);
}
...
TEST_F(VolumeMetaIntfFixture, SaveVolumes_testIfAnErrorCodeWillBeReturnedWhenTheFileCannotBeCreated)
{
    ...
    EXPECT_EQ(VolumeMetaIntf::SaveVolumes(list, ARRAY_NAME, ARRAY_ID, file), expectValue);
}
