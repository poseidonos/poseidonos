...
public:
    ...
    MOCK_METHOD(spdk_nvmf_subsystem*, FindSubsystemWithArrayName, (const string& subnqn, string arrayName), (override));
    MOCK_METHOD(bool, TryToAttachNamespace, (const string& nqn, int volId, string& arrayName, uint32_t& nsId, uint64_t time), (override));
    MOCK_METHOD(bool, CheckSubsystemExistance, (string arrayName), (override));
