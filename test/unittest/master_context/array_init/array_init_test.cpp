...
TEST_F(ArrayInitTest, InitTest)
{
    Array& array = *Array::Instance();

    DeviceSet<UBlockDevice *> devices;
    vector<UBlockDevice *> devs = DeviceManager::Instance()->GetDevs();
    
    vector<UBlockDevice *>::iterator it = devs.begin();
    devices.nvm = *it;
    it++;

    devices.data.clear();
    for (uint64_t index = 0; index < 4; index++, ++it)
    {
        devices.data.push_back(*it);
    }

    for (uint64_t index = 0; index < 1; index++, ++it)
    {
        devices.spares.push_back(*it);
    }
    
   
    EXPECT_TRUE(ArrayStatus::DELETED == array.GetStatus());
    EXPECT_TRUE(0 == array.Create(devices));
    EXPECT_TRUE(ArrayStatus::CREATED == array.GetStatus());

    EXPECT_TRUE(0 == array.AddSpare(*it));
    ...
    if (0 == ret)
    {
        EXPECT_TRUE(ArrayStatus::MOUNTED == array.GetStatus());
        EXPECT_TRUE(0 == array.Unmount());
        EXPECT_TRUE(ArrayStatus::CREATED == array.GetStatus());
    }
    else
    {
    ...
    EXPECT_TRUE(0 == array.Delete());
    EXPECT_TRUE(ArrayStatus::DELETED == array.GetStatus());

}


TEST_F(ArrayInitTest, LoadTest)
{
...
TEST_F(ArrayInitTest, WriteTest)
{
#if 0
