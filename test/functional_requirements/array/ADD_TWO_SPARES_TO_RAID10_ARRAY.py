...
def check_result():
    if api.is_spare_device(ARRAYNAME, SPARE1) == True:
        if api.is_spare_device(ARRAYNAME, SPARE2) == True:
            return "pass"
    return "fail"

def execute():
    MOUNT_RAID10_ARRAY_NO_SPARE.execute()
    out = cli.add_device(SPARE1, ARRAYNAME)
    print (out)
    out = cli.add_device(SPARE2, ARRAYNAME)
    print (out)
    return out
