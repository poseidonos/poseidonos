...
echo -[Map : get_segment_inv_count]------------------------------------------
${BIN_DIR}/cli wbt get_segment_inv_count --output segInvCount.bin --json > ${cliOutput}
check_result
...
do    
    ...
    ${BIN_DIR}/cli wbt write_raw --dev unvme-ns-${count} --lba ${lbaIdx} --count ${lbaCnt} --pattern 0xdeadbeef --output segInvCount.bin --json > ${cliOutput}
    check_result 
    ...
    if [[ "$ip" =~ "$VM_IP_RANGE_1" ]] || [[ "$ip" =~ "$VM_IP_RANGE_2" ]]; then
        ...
        echo -[IO Path : unvme-ns-${count} : wbt write_uncorrectable_lba ]------------------------------------------
        ${BIN_DIR}/cli wbt write_uncorrectable_lba --dev unvme-ns-${count} --lba ${lbaIdx} --output segInvCount.bin --json > ${cliOutput}
        check_result
    fi

    echo -[IO Path : unvme-ns-${count} : wbt flush]------------------------------------------
    ${BIN_DIR}/cli wbt flush --output segInvCount.bin --json > ${cliOutput}
 
    check_result

    echo -[IO Path : unvme-ns-${count} : wbt read_raw]------------------------------------------
    ${BIN_DIR}/cli wbt read_raw --dev unvme-ns-${count} --lba ${lbaIdx} --count ${lbaCnt} --output dump.bin --output segInvCount.bin --json > ${cliOutput}
