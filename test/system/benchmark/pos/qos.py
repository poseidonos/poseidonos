...
def CheckTestValidity(test_name, testcase):
    ...
    if count < 3:
        lib.printer.red(
            f"TEST NAME:{test_name}'s syntax is incorrect. Argument less than 3")
        return False

    limit_type = testcase[0]
    if limit_type != "reset" and limit_type != "bw" and limit_type != "iops":
        lib.printer.red(
            f"TEST NAME:{test_name}'s limit type is wrong (reset, bw, iops)")
        return False
    ...
    if type(testcase[1]) is list:
        ...
        if limit_type != "reset" and (len(testcase[1]) != len(testcase[2])):
            lib.printer.red(
                f"TEST NAME:{test_name}'s number of volume list & limit value is not the same")
            return False
    else:
        limit_how = testcase[1]
        if limit_how != "rate" and limit_how != "value":
            lib.printer.red(
                f"TEST NAME:{test_name}'s limit how is wrong (rate, value)")
            return False
    return True
...
def GetQosCommandOption(testcase, base_perf):
    ...
    if (len(testcase) == 4 and testcase[3] == "min"):
        min_flag = True
    limit = {"type": "", "how": "", "value": 0.0,
             "min": min_flag}  # unit: kiops, MB/s
    ...
    if type(testcase[1]) is list:
        ...
        if limit["how"] == "rate" and limit["value"] != 0:
            ...
            limit["value"] = GetLimitPerformance(
                base_perf, limit["type"], limit_rate)
            if limit["type"] == "iops":
                limit["value"] /= 1000.0
        return limit
            ...
            for array in test_target.array_volume_list.keys():
                for vol_name in test_target.array_volume_list[array]:
                    if (-1 == pos.cli.set_qos(test_target.id, test_target.pw, test_target.nic_ssh, test_target.pos_cli, test_target.pos_dir, array, vol_name, limit_type, volume_limit_value, min_flag)):
                        lib.printer.red(
                            f"Fail to setting qos array : {array} vol : {vol_name}")
                        return False
        else:
...
def SetQosToAllVolumes(test_target, limit):
    ...
    if limit_type == "iops":
        ...
        lib.printer.green(
            f" Throttle pos's {limit_type} to {total_limit_value}MB/s")
...
def GetResultQos(test_vdbench, init_name, vd_disk_names, limit_type, workload_name):
    ...
    for disk_name in vd_disk_names:
        file_name = test_vdbench.CopyHtmlResult(
            vd_disk_names[disk_name], init_name)
        json_result_file = test_vdbench.ParseHtmlResult(
...
def CheckQos(json_result_file, limit_type, expected_value, prev_expected_value, base_perf, min_flag=False):
    ...
    for i in range(0, min(read_line_num, array_len)):
        ...
        if (expected_value == 0):
            print(
                f"  BW: {bw_value} MB/sec, IOPS: {iops_value}k, actual value : {actual_value}")
        else:
            print(
                f"  BW: {bw_value} MB/sec, IOPS: {iops_value}k, actual value : {actual_value}, expected_value : {expected_value}")
        ...
        if expected_value == -1:  # check if the same as prev perf
            if (prev_expected_value[limit_type] * 0.9 > actual_value) or (prev_expected_value[limit_type] * 1.1 < actual_value):
                print(
                    f" Failed qos throttling: expected_value = {prev_expected_value[limit_type]} with o/h +-10% , actual_value = {actual_value}")
                result[0] = False
            ...
            if (base_perf[limit_type] * 0.9 > actual_value) or (base_perf[limit_type] * 1.1 < actual_value):
                print(
                    f" Failed reset qos throttling: expected_value = {base_perf[limit_type]} with o/h +-10% , actual_value = {actual_value}")
                result[0] = False
        elif (actual_value * sign > expected_value * sign):
            print(
                f" Failed qos throttling: expected_value = {expected_value}, actual_value = {actual_value}")
            result[0] = False
...
def CheckEachVolume(init_name, volume_limit_list, vd_disk_names, test_vdbench, workload_name, volume_id_list, min_flag=False):
    ...
    for disk_name in vd_disk_names:
        ...
        print(
            f" -- Initiator: {init_name} , disk name: {disk_name} , volume id: {volId} -- ")
        throttling_success = CheckThrottledResult(
            limit_type, volume_list, volId, json_file, min_flag)
