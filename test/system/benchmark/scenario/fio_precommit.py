...
def play(json_targets, json_inits, json_scenario):  # player.py에서 호출하는 method로 이름/인자 형식을 준수!
    ...
    if not skip_workload:
        ...
        test_target = targets[next(iter(targets))]      # targets 딕셔너리에 첫 번째 객체 정보를 가져온다
        test_init = initiators[next(iter(initiators))]  # initiators 딕셔너리에 첫 번째 객체 정보를 가져온다
        test_fio = fio.manager.Fio(test_init.id, test_init.pw, test_init.nic_ssh)  # fio 객체 생성
        graph_fio = graph.manager.Fio(f"{json_scenario['OUTPUT_DIR']}/{now_date}_precommit_fio")  # graph 객체 생성
        ...
        test_fio.opt["log_avg_msec"] = "1000"   # write_xxx_log가 출력되는 주기는 1초로 설정
        ...
        for bs in block_size:
            ...
            for rw in readwrite:
                ...
                test_fio.opt["write_bw_log"] = f"{test_fio.opt['output']}"    # job별로 구분되어 저장
                test_fio.opt["write_iops_log"] = f"{test_fio.opt['output']}"  # job별로 구분되어 저장
                test_fio.opt["write_lat_log"] = f"{test_fio.opt['output']}"   # job별로 구분되어 저장
                ...
                graph_fio.AddEtaData(f"{test_fio.opt['output']}.eta", f"{bs}_{rw}")
                ...
                graph_fio.AddLogData(json_scenario['OUTPUT_DIR'], f"{now_date}_precommit_fio_{bs}_{rw}")
                ...
                graph_fio.DrawLog(f"{json_scenario['OUTPUT_DIR']}/{now_date}_precommit_fio_{bs}_{rw}")
