...
verify=False #multiple write does not support verify option.
...
# extra fio options
extra_fio_options=" --thread=1 --group_reporting=1 --direct=1 --numjobs=3 --ramp_time=0 --time_based=1"
...
def run_fio(io_size_bytes, block_size, qd, rw_mix, cpus_allowed, run_num, workload, run_time_sec):
    ...
    if file_num > 1:
        for i in range(0, file_num):
            if fio_plugin == "nvme":
                command += " --name=test" + str(i) + " --filename='trtype=tcp adrfam=IPv4 traddr=" + str(traddr) + " trsvcid=1158 subnqn=nqn.2019-04.ibof\:subsystem" + str(i+1) + " ns=1'"
            else:
...
for test in range(0, iter_num):
    for i, (s, b, q, m, w, c, t) in enumerate(itertools.product(io_size, block_size, q_depth, mix, readwrite, cpus_allowed, run_time)):
        test_count = 0
        run_fio(s, b, q, m, c, test_no, w, t)
        if profile_mode == True:
            parse_results(s, b, q, m, c, test_no, w, t)
        test_count = test_count + 1
    test_no = test_no + test_count
