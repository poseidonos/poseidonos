...
############################################################################
def overwrite(volumeId, offset, size, pattern):
    for pattern_idx in range(0, len(TEST.patterns)):
        TEST_FIO.write(volumeId, offset, size, pattern_idx + volumeId)
...
def test(offset, size, testIdx):
    ...
    for volId in volumes:
        th = Thread(target=overwrite, args=(volId, offset, size, testIdx + volId))
    ...
    for volId in volumes:
        TEST_FIO.verify(volId, offset, size, -1 + volId)
