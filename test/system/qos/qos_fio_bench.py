...
def run_fio(io_size_bytes, block_size, qd, rw_mix, cpus_allowed, run_num, workload, run_time_sec):
    extra_fio_options=" --thread=1 --group_reporting=" + str(group_reporting) +  " --direct=1"
    ...
    # call fio
    if workload_type == 0:
        readwrite=['read', 'read', 'read', 'read', 'read', 'read', 'read', 'read']
        blocksize=['128k', '128k', '128k', '128k', '128k', '128k', '128k', '4k']
    if workload_type == 1:
        readwrite=['write', 'write', 'write', 'write', 'write', 'write', 'write', 'write']
        blocksize=['128k', '128k', '128k', '128k', '128k', '128k', '128k', '4k']
    if workload_type == 2:
        readwrite=['read', 'read', 'read', 'read', 'write', 'write', 'write', 'write']
    ...
    if block_size.find('-') == -1:
        command += " --bs_unaligned=0 --bs=" + str(block_size)
    else:
...
def parse_argument():
    ...
    if(args.offset!=None):
        offset=args.offset
    if(args.workload_type!=None):
        workload_type=int(args.workload_type,0)
    if(args.group_reporting!=None):
