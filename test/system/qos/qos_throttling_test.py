...
def get_performance_bandwith_MiB():
    global args
    initiator_script = args.ibofos_root + "/test/system/qos/initiator_parse_fio_result.py"
    result = remote_execute(args.initiator_ip, args.initiator_id, args.initiator_pw, initiator_script)
    print ("result : %s " % result.rstrip("\n") + "MiB/s")
    return result.rstrip("\n")

def execute_fio_in_initiator(readwrite, block_size, qd):
    global args
    print ("Execute fio at initiator readwrite=%s block_size=%s qd=%s" % (readwrite, block_size, qd))
    ramp_time = "15"
    run_time = "5"
    file_num = "1"
    num_jobs = "31"
    ...
    remote_execute(args.initiator_ip, args.initiator_id, args.initiator_pw, "rm -rf " + result_file)
...
def cli_max_bw_setting(max_bw, ratio):
    ...
    cli_cmd = ibofos_root + cli + " -a POSArray --volume-name vol1 --maxbw " + str(int(max_bw * ratio))
    print (cli_cmd)
...
def cli_max_iops_setting(max_iops, ratio):
    ...
    cli_cmd = ibofos_root + cli + " -a POSArray --volume-name vol1 --maxiops " + str(int(max_iops * ratio))
    print (cli_cmd)
    ret = os.system(cli_cmd)
    print ("return code : %d" % ret)
    return ret
...
def test_scenario():
    ...
    if (os.path.isfile(config_dir + "ibofos.conf")):
        os.system("cp " + config_dir + "ibofos.conf " + config_dir +"ibofos.conf.reserved -rf")
    os.system("cp " + ibofos_root + "config/ibofos_for_perf_ci.conf " + config_dir + "ibofos.conf -rf")
    ...
    for index in range(0, 4):
        max_bw = execute_fio_in_initiator(readwrite[index], block_size[index], io_depth[index])
        max_iops = int(float(int(max_bw)) / int(block_size[index].rstrip("k")))*1024
        for ratio in ratios:
            cli_max_bw_setting(int(0), ratio)
            ret = cli_max_bw_setting(int(max_bw), ratio)
            bw = "0"
            if (int(int(max_bw) * ratio) >= 10 and ret == 0):
                bw = execute_fio_in_initiator(readwrite[index], block_size[index], io_depth[index])
            dict_str = readwrite[index] + " io_depth=" + io_depth[index] + " block_size=" + block_size[index] + " ratio=" + str(ratio)
            ...
            dict_real_iops[dict_str] = int(float(int(bw)) / int(block_size[index].rstrip("k")))
            cli_max_iops_setting(int(0), ratio)

    print ("############ Result ##############")
    for key in dict_real:
        print (key, "BW setting = ", dict_expect[key], "MiB/s", "  real = ", dict_real[key], "MiB/s  IOPS setting = ", dict_expect_iops[key], "kIOPs  real = ", dict_real_iops[key], "kIOPs")
