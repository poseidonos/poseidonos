...
def _GenerateMocks(filename, source, ast_list, desired_class_names):
  ...
  lines = []
  extended_namespace = False

  # Add header file before the namespace
  try:
    lines.extend(['#include <gmock/gmock.h>'])
    lines.extend(['#include <string>'])
    lines.extend(['#include <list>'])
    lines.extend(['#include <vector>'])
  ...
  for node in ast_list:
    if (isinstance(node, ast.Class) and node.body and
      ...
      if (class_node.namespace and not extended_namespace):
        lines.extend(['namespace %s' % n for n in class_node.namespace])  # }
        lines.append('{')
        extended_namespace = True
      ...
      lines.append('public:')
      # Add using line
      lines.append('%susing %s::%s;' % (' ' * _INDENT, class_name, class_name))
      # Add all the methods.
  ...
  # TODO(kyuho): what if namespace is not declared in the .h file...?
...
def run(filepath):
  ...
  try:
    ...
    lines = _GenerateMocks(filepath, source, entire_ast, desired_class_names)
    return lines
