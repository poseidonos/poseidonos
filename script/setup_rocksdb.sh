...
function collect_devices() {
	...
	local ids dev_type dev_id bdf bdfs in_use driver cnt_userdevice
	...
	while read -r _ dev_type dev_id; do
		...
		for bdf in "${bdfs[@]}"; do
			in_use=0
			if [[ $bdf == $nvme_1 || $bdf == $nvme_0 ]] && [[ $mode != "reset" ]]; then
				echo "Use Kernel driver : $bdf"
				continue
			fi
			if [[ $1 != status ]]; then
			...
			num_userdatdev=$(( $num_userdatdev + 1 ))
		done
